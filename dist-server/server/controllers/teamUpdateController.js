...
const canAccessTeam = async (teamId, userId, role) => {
    if (role === 'admin')
        return true;
...
const createTeamActivity = async (teamId, actor, message) => {
...
export const createTeamUpdate = async (req, res) => {
    try {
        ...
        const { progressPercent, status, note, blockers } = req.body;
...
export const addTeamUpdateComment = async (req, res) => {
    try {
        ...
        update.comments.push({ user: req.user?._id, text, createdAt: new Date() });
        await update.save();
        const populatedUpdate = await TeamUpdate.findById(update._id)
            .populate('submittedBy', 'name email')
            .populate('comments.user', 'name email');
