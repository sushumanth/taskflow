...
const isTeamMemberOrLead = async (teamId, userId) => {
    return Team.exists({
        _id: teamId,
...
export const createTeam = async (req, res) => {
    try {
        ...
        const normalizedMembers = (memberUserIds || []).filter((id) => id && id !== leadUserId);
...
export const getTeams = async (req, res) => {
    try {
        ...
        const { status, leadUserId } = req.query;
        const query = {};
        if (status)
            query.status = status;
        if (leadUserId)
            query.leadUserId = leadUserId;
...
export const changeTeamLead = async (req, res) => {
    try {
...
export const assignTaskToTeam = async (req, res) => {
    try {
        const { taskId, teamAssignment } = req.body;
        const teamId = getParamId(req.params.id);
        if (!teamId) {
            res.status(400).json({ success: false, message: 'Team id is required' });
            return;
        ...
        if (!team.assignedTasks.some((id) => id.toString() === taskId)) {
            team.assignedTasks.push(taskId);
            await team.save();
        }
        await createTeamActivity({
            teamId,
...
export const assignProjectToTeam = async (req, res) => {
    try {
        const { projectId, teamAssignment } = req.body;
        const teamId = getParamId(req.params.id);
        if (!teamId) {
            res.status(400).json({ success: false, message: 'Team id is required' });
            return;
        ...
        if (teamAssignment)
            project.teamAssignment = teamAssignment;
        await project.save();
...
export const getTeamPerformanceSummary = async (req, res) => {
    try {
        const teamId = getParamId(req.params.id);
        if (!teamId) {
            res.status(400).json({ success: false, message: 'Team id is required' });
