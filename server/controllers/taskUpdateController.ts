...
const getQueryValue = (param: string | string[] | undefined): string | undefined => {
  if (Array.isArray(param)) return param[0];
  return param;
};

const canViewTask = async (taskId: string, userId: string, role?: string): Promise<boolean> => {
...
const createActivity = async (data: {
  ...
  updateId?: string;
  actor: string;
  type: 'update_submitted' | 'update_reviewed' | 'comment_added' | 'update_edited' | 'update_deleted';
  message: string;
}) => {
...
export const createTaskUpdate = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const normalizedStatus = status as 'todo' | 'in-progress' | 'review' | 'done' | 'rejected';
    ...
    const files = (req as AuthRequestWithFiles).files || [];
...
export const getTaskUpdatesByTask = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const taskId = getParamId(req.params.id);
    if (!taskId) {
      res.status(400).json({ success: false, message: 'Task id is required' });
...
export const getTaskUpdatesByEmployee = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const employeeId = getQueryValue(req.query.employeeId as string | string[] | undefined);
    const userId = req.user?._id?.toString();
    if (!userId) {
      res.status(401).json({ success: false, message: 'Not authorized' });
      return;
...
export const reviewTaskUpdate = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    update.review = {
      status: reviewStatus as 'approved' | 'rejected' | 'changes-requested',
...
export const addTaskUpdateComment = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const updateId = getParamId(req.params.id);
    if (!updateId) {
      res.status(400).json({ success: false, message: 'Update id is required' });
...
export const editTaskUpdate = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const updateId = getParamId(req.params.id);
    if (!updateId) {
      res.status(400).json({ success: false, message: 'Update id is required' });
